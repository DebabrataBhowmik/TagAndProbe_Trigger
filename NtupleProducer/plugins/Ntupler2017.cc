#include "TagAndProbe_Trigger/NtupleProducer/interface/Ntupler2017.h"
...
//
Ntupler2017::Ntupler2017(const edm::ParameterSet& iConfig):
...
void
Ntupler2017::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  using namespace std;
  using namespace edm;
     ...
     for(unsigned i=0; i < muons->size();++i ) {
      ...
      // Trigger matching
       for (unsigned int iteTrigObj = 0 ; iteTrigObj < filterToMatch_.size() ; iteTrigObj++){
          ...
          if(mu_filters[1].Contains(filter) && foundTheLeg && legObjects[iteTrigObj].at(iPass).pt()>=12 && delRL1_7_L10p3 < 0.3 )  {filterMu12_Ele23_legMu_L10p3 = true; }
          if(mu_filters[1].Contains(filter) && foundTheLeg && legObjects[iteTrigObj].at(iPass).pt()>=12 && delRL1_7_L10p5 < 0.5 )  {filterMu12_Ele23_legMu_L10p5 = true; }
          if(mu_filters[1].Contains(filter) && foundTheLeg && legObjects[iteTrigObj].at(iPass).pt()>=23 &&  delRL1_23_L10p3 < 0.3 ) {filterMu23_Ele12_legMu_L10p3 = true; }
          if(mu_filters[1].Contains(filter) && foundTheLeg && legObjects[iteTrigObj].at(iPass).pt()>=23 &&  delRL1_23_L10p5 < 0.5 ) {filterMu23_Ele12_legMu_L10p5 = true; }
...
/*
void 
Ntupler2017::beginRun(edm::Run const&, edm::EventSetup const&)
{
}
*/
...
/*
void 
Ntupler2017::endRun(edm::Run const&, edm::EventSetup const&)
{
}
*/
...
/*
void 
Ntupler2017::beginLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&)
{
}
*/
...
/*
void 
Ntupler2017::endLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&)
{
}
*/
...
void
Ntupler2017::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
int Ntupler2017::matchToTruth(const edm::Ptr<reco::GsfElectron> el, 
  ...
  if( ancestorPID == -999 && ancestorStatus == -999 ){
    ...
    // Complain.
    printf("Ntupler2017: ERROR! Electron does not apper to have a non-electron parent\n");
    return UNMATCHED;
  }
...
bool Ntupler2017::cmd(const reco::GenParticle & s1, const reco::GenParticle & s2)
{  
   return s1.pt() > s2.pt();
}

void Ntupler2017::findFirstNonElectronMother(const reco::Candidate *particle,
						 int &ancestorPID, int &ancestorStatus){

  if( particle == 0 ){
    printf("Ntupler2017: ERROR! null candidate pointer, this should never happen\n");
    return;
  }
...
bool
Ntupler2017::hasWZasMother(const reco::GenParticle  p)  
{
    bool foundZ = false;
...
/*void Ntupler2017::printCutFlowResult(vid::CutFlowResult &cutflow){
